...
public class SysUserServiceImpl extends BaseServiceImpl<SysUser, String> implements SysUserService {
    ...
    public void setMenuAndRoles(String username) {
        ...
        // 查询所有租户信息
          // 譬如段是管理员还是租户
        List<Map> tenants;
        QuanPrint.print("用户的id为："+currentUser.getId());
        if(currentUser.getId().equals("acfc0e9232f54732a5d9ffe9071bf572")) {
            // 如果是管理员就走管理员查询
            System.out.println("管理员登录");
            tenants = sysUserMapper.getTenants();
        } else {
            // 如果是租户就走租户查询
            System.out.println("租户登录");
            tenants = sysUserMapper.getUserTenants(currentUser.getId());
        }
        System.err.println("SysUserServiceImmpl => setMenuAndRoles():登陆查询tenants"+tenants);
        // 放到session中
     ...
     */
    @Override
    public List<ProjectTemp> getProject(String id) {
        List<ProjectTemp> project = sysUserMapper.getProject(id);
//        List<ProjectTemp> project = new ArrayList<>();

        QuanPrint.print("sysUserServiceImpl => getProject() 获取到的参数为："+id+" 查询到的project为："+project);
        return  project;
    }
}
